...
- **Tracking issue cadence**: issues open just-in-time. The roadmap is the long-form planning artifact; issues are the work artifacts. Status: #25-#27 closed (v1.3-v1.5), v1.5.1 patch shipped 2026-05-19 without a tracking issue (patches don't get one per convention), #38 closed (v1.6, 2026-05-20), #39 closed (v1.7, 2026-05-20), #40 closed (v1.8, 2026-05-20), #41 closed (v1.9, 2026-05-20), #42 closed (v1.10, 2026-05-21), #44 closed (v1.11, 2026-05-21), #43 closed (v1.12, 2026-05-22), #45 closed (v1.13, 2026-05-25), #46 closed (v1.14, 2026-05-25), #47 closed (v1.15, 2026-05-25); v1.15.1 patch shipped 2026-05-26 without a tracking issue (patches don't get one per convention; closed 11 audit findings — 5 image-tag / operator-publishing / terraform / image-size-budget ship-blockers + 6 UX gaps in the demo seeder, render --format=ocsf alias, checks list --provider=k8s alias, plugins-list unsigned hint, /settings landing, and the scripts/install.sh-vs-deploy/install.sh duplication). Issue numbers from the v1.11-v1.15 batch came in out of version order — the real mapping is #44=v1.11, #43=v1.12, #45=v1.13, #46=v1.14, #47=v1.15. #56 closed (v1.16, 2026-05-27 — Mobile / PWA: 8 phases shipping manifest + service worker + install banner + xs: breakpoint + .ck-table-cards mobile-cards pattern + VAPID Web Push + /quick-scan + bottom action bar + swipe gestures + offline banner). #57 closed (v1.17, 2026-05-27 — Data warehouse bridges: 8 phases, Parquet + NDJSON writers, BigQuery + Snowflake + Redshift loaders, OpenLineage emit on scan lifecycle, snapshot API, scheduled-sync infrastructure; 4 cloud-warehouse deps added; migrations 0027 + 0028). #58 closed (v1.18, 2026-05-28 — Design system & visual polish: 13 phases, ADR-017 design-system contract, internal/server/ui/design/ (tokens.css + 25-component partial library + 30 empty-state SVGs + ~100-symbol sprite + /design route), MetricCard, InfoTooltip audit, depth/gradient/motion/typography tokens, nprogress + skeletons, toast queue + optimistic UI, avatars + status pills, chart rich-tooltip, zero-critical confetti + celebration card + streak badges, CK_BRAND_PRIMARY brand kit; no pkg/compliancekit change, no new migrations — still 28). #59 closed (v1.19, 2026-05-29 — Onboarding 2.0 + global search + table excellence: 10 phases, feature-tour overlay (migration 0029) + /onboarding, changelog modal, in-app feedback widget (migration 0030) + /admin/feedback, first-run tour + empty-state coaching v2, screenshot-grade demo seed (~150 resources × 8 scans → ~500 findings), global search index (internal/server/search, fuzzy + recency) + GET /api/v1/search + new pkg/compliancekit SearchType/SearchResult/SearchResponse, grouped Cmd-K/"/" search UI, Table 2.0 (migration 0031 user_table_state, resize/reorder/pin/visibility), inline edit + bulk-actions (migration 0032 findings.triage_status + note, POST /findings/bulk with audit_log entries), detail-panel resize/detach/j-k nav + per-page ? shortcuts; 1 new dep lithammer/fuzzysearch; migrations through 32). v1.19.1 patch shipped 2026-05-29 without a tracking issue (per convention; v2.0 phase-0 audit findings — AUDIT-1 serve-bootstrap subcommands didn't MigrateUp on a fresh DB; AUDIT-2 the whole deploy/ tree had drifted to 1.15.x across v1.16–v1.19 + no guard — fixed via openMigratedStore + a deploy/scripts/version-check.sh release gate). #60 closed (v2.0, 2026-05-29 — API-surface retro, **ADR-020**: no ABI break needed; the v1.x additive discipline held across all 121 public symbols; no /v2 library cut — the Go module keeps its v1 import path — but `v2.0.0` IS tagged as the binary/product line-opener, decoupled from the library module major (`compliancekit version`→v2.0.0; embed→v1 path / v1.x library versions); the next real break triggers the /v2 cut). All v1.x tracking issues (#5-#27, #38-#47, #56-#60) now closed; ADR-016's v1.x polish reservation (v1.0–v1.19) is complete. v2.1+ (multi-tenant onward) open just-in-time on the unchanged v1 module path; v2.1+ stay in ROADMAP table form until kickoff. v2.0.1 patch shipped 2026-05-29 without a tracking issue (per convention): fixed the findings MetricCard severity row leaking raw `&lt;section …` text — the ck-metric-card partial used a dynamic `<{{ $tag }}>` element name html/template's autoescaper can't analyze; replaced with explicit `<a>`/`<section>` branches + a regression test pinning unescaped output (the v1.18 grep checks were fooled because class substrings appear in the escaped form too). Kicks off the **v2.0.x app-performance + UI-render-correctness hardening** patch series (ROADMAP near-term row): a render-correctness CI sweep (every authed page asserts no `&lt;` leak / `ZgotmplZ`), SSE "RECONNECTING"-badge + first-paint polish, and a findings-explorer query/index review under real-fleet row counts.
...
- **Forward references**: ADR-021 reserved for the v2.13 docs-site-engine pick (Hugo vs Mkdocs Material) at v2.13 kickoff. Plugin host model decision (ADR open question from line 661) defers to v2.9 kickoff per existing convention.
...
## ADR-020 — v2.0 contract retro: no ABI break; tag v2.0.0 as the binary line-opener; library module stays at v1
**Date:** 2026-05-29
**Status:** Accepted

### Question
v2.0 (#60) is the reserved API-surface-refinement checkpoint. Its phase 0 is a retro: audit every `pkg/compliancekit` extension shipped across v1.0 → v1.19 against the original v1.0 contract (ADR-014), and decide whether the v1.x learnings require a *non-additive* (breaking) change. The DoD branches: a real `/v2` module cut, or "no break needed → close with a reservation note + tag `v2.0.0` as a re-export of the v1.19 surface for SemVer consistency." The retro found no break — so the open question narrows to: how do we honor the "tag v2.0.0" DoD when Go's module rules forbid a v2+ tag on a module whose path lacks `/v2`?

### Decision
Two decisions, decoupled:

1. **The Go *library* module stays at v1.** No `pkg/compliancekit/v2/` cut; `go.mod` remains `module github.com/darpanzope/compliancekit`; `pkg/compliancekit` keeps its v1 import path. The retro found no breaking-change requirement, so a `/v2` cut would be pure churn.

2. **The *binary/product* line opens at `v2.0.0`.** We tag `v2.0.0` (a normal goreleaser release of the current code) to mark the start of the v2.x arc, per the #60 DoD. The binary's version and the library module's major are **explicitly decoupled**: the daemon/CLI is `v2.0.0`; the embeddable library continues to be imported at the unchanged v1 path. `go get github.com/darpanzope/compliancekit@v2.0.0` is *not* a supported library operation (Go requires a `/v2` module path for that) — embedders import `…/pkg/compliancekit` and pin a `v1.x.y` library version, which is unchanged and fully compatible.

The v2.0 ROADMAP slot's *original* intent (a reserved ABI-break cycle) is preserved as the **next** real `/v2` trigger, whenever a concrete v2.x feature (multi-tenant, risk-score, …) first needs it.

### Reasoning
- **ADR-014's additive discipline held for 19 minors.** The retro audited the full public surface (121 symbols across the root package + the `plugin` / `rbac` / `rules` subpackages). Every v1.x addition — v1.8 `Comment` + `User`, v1.9 `rules` (Rule/Condition/Term/Op/Trigger), v1.12 `rbac` (Role/Set/Permission/Action/Resource), v1.13 `plugin` (Manifest/Plugin/Catalog/Kind), v1.19 `SearchType`/`SearchResult`/`SearchResponse` — landed additively, gated the whole way by `make api-check`. Nothing in the v1.0 core (`Finding` / `Resource` / `ResourceGraph` / `Check` / `Registry` / `Severity` / `Status` / the `Reporter`/`Collector`/`Evaluator` interfaces) was renamed or reshaped.
- **The only warts are cosmetic + already disambiguated by package qualification.** `rbac.Resource` (a permission subject) shares a name with the top-level `compliancekit.Resource` (a scanned cloud resource); `rbac.Action` and `rules.Action` are both `Action` in their own packages; `SearchResult.Score` is a documented ordering hint. None confuse a caller who imports with package qualifiers, and none justify the cost of a `/v2` cut.
- **A `/v2` cut imposes real cost for no functional gain.** Go's module-versioning convention requires v2+ to live under a `/v2` import path with `module …/v2` in `go.mod`. Cutting `/v2` purely to rename `rbac.Resource → rbac.Subject` would force every embedder to rewrite import paths + force the project to dual-maintain the v1 + v2 surfaces through a grace window — a multi-file, multi-release tax to buy cosmetic clarity. Not worth it.
- **Binary version ≠ library module major — and that's normal for a binary-first tool.** compliancekit's primary distribution is binaries (Homebrew, GHCR, release archives); the embeddable Go library is a secondary surface (ADR-014). It is standard practice for a binary-first project to advance its product/binary major (v1.x → v2.0.0) to signal a new line while keeping the embedded library on its existing import path. The two version namespaces are independent: `compliancekit version` reports `v2.0.0`; `import ".../pkg/compliancekit"` resolves a `v1.x` library release. This is what the #60 DoD asked for ("tag v2.0.0 … for SemVer consistency") read correctly through Go's module rules.
- **Honors the issue.** #60's DoD for the no-break branch is "close with a reservation note **and** tag `v2.0.0` as a re-export of the v1.19 surface." Both happen: ADR-020 is the reservation note; `v2.0.0` is the tag. The only refinement is documenting *why* the tag is a binary-line marker, not a library-module bump.

### Rejected alternatives
- **Cut `pkg/compliancekit/v2/` now to fix the naming smells.** Rejected: forces every embedder through an import-path rewrite + a dual-maintenance grace window to buy cosmetic clarity that package qualification already provides. A `/v2` cut should be driven by a real semantic break (e.g. v2.1 multi-tenant reshaping `Finding`, or v2.8 risk-score changing the score contract), not by tidiness.
- **Don't tag `v2.0.0` at all; close v2.0 as a pure doc reservation on the v1.x line.** Rejected: the #60 DoD explicitly asks for the `v2.0.0` tag, and a tagged release cleanly opens the v2.x product arc for binary consumers (Homebrew/GHCR/release page). Skipping the tag to dodge the (irrelevant-to-binary-consumers) `go get` caveat under-delivers the milestone.
- **Bump the library module to `/v2` *and* tag `v2.0.0` so `go get @v2.0.0` works.** Rejected: that *is* a `/v2` cut — exactly the churn the retro found unjustified. We don't move the module path without a real break.
- **Rename the cosmetic smells additively (add `rbac.Subject` aliasing `rbac.Resource`, deprecate the old name).** Considered + deferred: doable without a major bump, but it grows the surface with duplicate symbols for marginal benefit. Fold it into the next real break.

### Consequences
- **`go.mod` module path unchanged**: `github.com/darpanzope/compliancekit`. Embedders do nothing; `import "github.com/darpanzope/compliancekit/pkg/compliancekit"` keeps working at its v1 library versions.
- **`v2.0.0` is tagged** as a normal goreleaser release of the post-v1.19.1 code (binaries + GHCR image + Homebrew formula + SBOM). It carries no code change vs v1.19.1 beyond this ADR + the doc sweep — it is a deliberate line-opening marker.
- **The library/binary version split is documented** in the README embed section + here: `compliancekit version` → `v2.0.0`; library embed → v1 import path, `v1.x` module versions. A future genuine break triggers the `/v2` module cut (reviving this ADR's deferred phases 1–5) and re-aligns the two namespaces.
- **`cmd/genapi` + `make api-check` continue to guard the single v1 surface.** No dual-surface walking is needed.
- **The naming-smell inventory** (`rbac.Resource`, `rbac.Action`/`rules.Action`, `SearchResult.Score`) is recorded here as the v2 cleanup backlog — to be applied when a real break next opens the `/v2` door.
...
- **Docs-site engine (Hugo vs Mkdocs Material):** decision at v2.13 kickoff (reserved ADR-021). Hugo has the lead on theme flexibility + build speed; Mkdocs Material wins on plugin ecosystem + search-out-of-the-box. Either choice keeps the multi-version-selector + pagefind invariant from the v2.13 deliverables.
