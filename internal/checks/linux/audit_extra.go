...
var auditRuleSpecs = []auditRuleSpec{
	{id: "linux-audit-rule-passwd", title: "auditd must watch /etc/passwd",
		severity: compliancekit.SeverityHigh, soc2: []string{"CC7.2"}, iso: []string{"A.8.15"}, cis: []string{"4.1.3.7"},
	...
	{id: "linux-audit-rule-group", title: "auditd must watch /etc/group",
		severity: compliancekit.SeverityMedium, soc2: []string{"CC7.2"}, iso: []string{"A.8.15"}, cis: []string{"4.1.3.7"},
	...
	{id: "linux-audit-rule-sudoers", title: "auditd must watch /etc/sudoers + /etc/sudoers.d",
		severity: compliancekit.SeverityHigh, soc2: []string{"CC6.3"}, iso: []string{"A.5.15"}, cis: []string{"4.1.3.20"},
	...
	{id: "linux-audit-rule-selinux", title: "auditd must watch /etc/selinux/ + /usr/share/selinux/",
		severity: compliancekit.SeverityMedium, soc2: []string{"CC6.6"}, iso: []string{"A.8.7"}, cis: []string{"4.1.3.14"},
	...
	{id: "linux-audit-rule-time-change", title: "auditd must watch time-change syscalls",
		severity: compliancekit.SeverityMedium, soc2: []string{"CC7.2"}, iso: []string{"A.8.17"}, cis: []string{"4.1.3.5"},
	...
	{id: "linux-audit-rule-localtime", title: "auditd must watch /etc/localtime",
		severity: compliancekit.SeverityLow, soc2: []string{"CC7.2"}, iso: []string{"A.8.17"}, cis: []string{"4.1.3.5"},
	...
	{id: "linux-audit-rule-lastlog", title: "auditd must watch /var/log/lastlog (login records)",
		severity: compliancekit.SeverityMedium, soc2: []string{"CC7.2"}, iso: []string{"A.8.15"}, cis: []string{"4.1.3.6"},
...
func auditRulesFromHost(h compliancekit.Resource) ([]string, bool) {
...
func auditRuleCheckFunc(spec auditRuleSpec) compliancekit.CheckFunc {
...
func auditRuleCheck(spec auditRuleSpec) compliancekit.Check {
...
func init() {
	for _, spec := range auditRuleSpecs {
		spec := spec
		compliancekit.Register(auditRuleCheck(spec), auditRuleCheckFunc(spec))
	}
}
