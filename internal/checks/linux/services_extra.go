...
var serviceMustRunSpecs = []serviceMustRunSpec{
	{id: "linux-service-time-sync-active", title: "Time-sync daemon must be running (chrony or systemd-timesyncd)",
		unit: "chronyd.service", severity: core.SeverityHigh,
	...
	{id: "linux-service-auditd-enabled", title: "auditd must be enabled at boot",
		unit: "auditd.service", severity: core.SeverityHigh,
	...
	{id: "linux-service-rsyslog-active", title: "rsyslog (or journald-forwarder) must be running",
		unit: "rsyslog.service", severity: core.SeverityHigh,
	...
	{id: "linux-service-cron-active", title: "cron daemon must be running (cron or cronie)",
		unit: "cron.service", severity: core.SeverityMedium,
...
var serviceMustNotRunSpecs = []serviceMustNotRunSpec{
	{id: "linux-service-avahi-disabled", title: "avahi-daemon must be disabled",
		unit: "avahi-daemon.service", severity: core.SeverityMedium,
	...
	{id: "linux-service-cups-disabled", title: "cups (print server) must be disabled",
		unit: "cups.service", severity: core.SeverityLow,
	...
	{id: "linux-service-dhcpd-disabled", title: "DHCP server must be disabled on non-DHCP hosts",
		unit: "isc-dhcp-server.service", severity: core.SeverityMedium,
...
var serviceMustAbsentSpecs = []serviceMustAbsentSpec{
	{id: "linux-service-telnet-absent", title: "telnetd must not be installed",
		unit: "telnetd.service", severity: core.SeverityHigh,
	...
	{id: "linux-service-rsh-absent", title: "rsh / rlogin / rexec must not be installed",
		unit: "rsh.service", severity: core.SeverityHigh,
	...
	{id: "linux-service-tftp-absent", title: "tftp server must not be installed",
		unit: "tftp-server.service", severity: core.SeverityHigh,
...
func servicesFromHost(h core.Resource) (docol.ServiceFacts, bool) {
	v, ok := h.Attributes["services"].(docol.ServiceFacts)
	return v, ok
}

func serviceCheckFunc(decide func(host string, present bool) (core.Status, string)) func(spec serviceCheckMeta) core.CheckFunc {
	return func(spec serviceCheckMeta) core.CheckFunc {
...
func mustRunCheck(spec serviceMustRunSpec) (core.Check, core.CheckFunc) {
	...
	fn := serviceCheckFunc(func(host string, ok bool) (core.Status, string) {
		if ok {
			return core.StatusPass, fmt.Sprintf("host %q: %s enabled + active", host, spec.unit)
		}
		return core.StatusFail, fmt.Sprintf("host %q: %s NOT enabled+active (must-run set)", host, spec.unit)
	})(meta)
...
func mustNotRunCheck(spec serviceMustNotRunSpec) (core.Check, core.CheckFunc) {
	meta := serviceCheckMeta{
		id: spec.id, unit: spec.unit, severity: spec.severity, tags: spec.tags,
		matches: func(s docol.ServiceFacts) bool { return !s.HasEnabled(spec.unit) && !s.HasActive(spec.unit) },
	}
	fn := serviceCheckFunc(func(host string, ok bool) (core.Status, string) {
		if ok {
			return core.StatusPass, fmt.Sprintf("host %q: %s neither enabled nor active", host, spec.unit)
		}
		return core.StatusFail, fmt.Sprintf("host %q: %s is enabled / active (must-not-run set)", host, spec.unit)
	})(meta)
	return makeServiceCheck(spec.id, spec.title, spec.severity, spec.soc2, spec.iso, spec.cis, spec.tags, spec.descSuffix, spec.scanner, spec.unit), fn
}

func mustAbsentCheck(spec serviceMustAbsentSpec) (core.Check, core.CheckFunc) {
	meta := serviceCheckMeta{
		id: spec.id, unit: spec.unit, severity: spec.severity, tags: spec.tags,
	...
	fn := serviceCheckFunc(func(host string, ok bool) (core.Status, string) {
		if ok {
			return core.StatusPass, fmt.Sprintf("host %q: %s absent (or masked)", host, spec.unit)
		}
		return core.StatusFail, fmt.Sprintf("host %q: insecure service %s present (must-absent set)", host, spec.unit)
	})(meta)
	return makeServiceCheck(spec.id, spec.title, spec.severity, spec.soc2, spec.iso, spec.cis, spec.tags, spec.descSuffix, spec.scanner, spec.unit), fn
}

func makeServiceCheck(id, title string, sev core.Severity, soc2, iso, cis, tags []string, descSuffix, scanner, unit string) core.Check {
	return core.Check{
