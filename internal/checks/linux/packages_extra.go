...
var manualPackageChecks = []manualVerifySpec{
	{id: "linux-pkg-gpg-keys-trusted-only", title: "Package manager must trust only documented signing keys",
		severity: compliancekit.SeverityHigh, soc2: []string{"CC6.7"}, iso: []string{"A.8.32"}, cis: []string{"1.2.1.1"},
	...
	{id: "linux-pkg-no-unattended-upgrades", title: "Auto-updates of security patches should be enabled",
		severity: compliancekit.SeverityMedium, soc2: []string{"CC7.1"}, iso: []string{"A.8.8"}, cis: []string{"1.9"},
	...
	{id: "linux-pkg-aide-installed", title: "AIDE (file integrity) should be installed",
		severity: compliancekit.SeverityMedium, soc2: []string{"CC7.2"}, iso: []string{"A.8.7"}, cis: []string{"6.1.1"},
	...
	{id: "linux-pkg-no-orphaned-packages", title: "Orphaned packages should be removed",
		severity: compliancekit.SeverityLow, soc2: []string{"CC6.6"}, iso: []string{"A.8.7"}, cis: []string{"1.9.2"},
	...
	{id: "linux-pkg-prelink-absent", title: "prelink must not be installed",
		severity: compliancekit.SeverityMedium, soc2: []string{"CC6.6"}, iso: []string{"A.8.7"}, cis: []string{"1.6.4"},
	...
	{id: "linux-mac-selinux-no-permissive-services", title: "No SELinux services should be in permissive mode",
		severity: compliancekit.SeverityMedium, soc2: []string{"CC6.6"}, iso: []string{"A.8.7"}, cis: []string{"1.7.1.5"},
	...
	{id: "linux-mac-apparmor-no-complain-mode", title: "AppArmor profiles must not be in complain mode (production)",
		severity: compliancekit.SeverityMedium, soc2: []string{"CC6.6"}, iso: []string{"A.8.7"}, cis: []string{"1.7.2.3"},
	...
	{id: "linux-pkg-cron-restricted-to-root", title: "cron.allow + at.allow must restrict to root (or specific users)",
		severity: compliancekit.SeverityLow, soc2: []string{"CC6.3"}, iso: []string{"A.5.15"}, cis: []string{"5.1.2"},
...
func init() {
	compliancekit.Register(CheckSELinuxEnforcing, SELinuxEnforcing)
	compliancekit.Register(CheckAppArmorActive, AppArmorActive)
