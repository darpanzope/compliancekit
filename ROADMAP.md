...
| ~~v1.18~~ ✅ | **Design system & visual polish** (v1.18.0 shipped 2026-05-28, #58) — 13 phase commits + new **ADR-017** codifying the design-system contract. `internal/server/ui/design/` is the canonical home: **tokens.css** (color / typography — font-sans/mono + --text-2xs..5xl + leading/tracking / motion — 4 durations 75/150/250/400ms + 6 Framer-style easings / shadow scale) as the single source of truth across light / dark / high-contrast palettes; **per-domain palette** extended to the full provider taxonomy (51 `--resource-{provider}-{kind}` + 6 `--brand-{provider}` tokens across AWS/GCP/DO/Hetzner/K8s/Linux); **gradient utility tokens** (`--gradient-{primary,critical,high,medium,low,success,info}`, dark-tuned); **25-component partial library** (`components/ck-*.html` + typed `args.go` structs, htmx + Alpine, shadcn-shaped API) composed into the daemon template tree; **MetricCard** hero primitive (gradient variants + decoration circle + trend arrow) shipped on the findings severity row; **InfoTooltip-on-every-card-title** audit across 9 dashboard pages; **named depth scale** (flat/raised/floating/glass + --shadow-glass + --blur-glass); **`/design` live component zoo** (28 component anchors + palette/easing/shadow galleries, canned data, the visual-regression target + CI gate that every component lands with a zoo entry); **page-top nprogress** + **skeleton-during-fetch** off the htmx lifecycle; **toast queue** (window.ck.toast — slide-in/stack/swipe-dismiss) + global htmx-error toasts + ckOptimistic helper (assignee widget optimistic-close); **30 theme-aware vanilla-SVG empty-state illustrations** + micro-interactions (button press-scale, card hover-lift); **hash-from-name gradient avatars** + status-pill system (scans, SSE-live) + **~100-symbol icon sprite** (`/assets/sprite.svg`); **chart rich-tooltip** on the score chart; **magic moments** (zero-critical confetti — the DoD — + celebration card + bronze/silver/gold streak badges); **brand kit** (CK_BRAND_PRIMARY primary-color override with WCAG-contrast validation, CSP-safe inline-style injection). No `pkg/compliancekit` surface change; no new schema migrations (still 28). | Every pixel feels intentional — Linear-tier polish |
| ~~v1.19~~ ✅ | **Onboarding 2.0 + global search + table excellence** (v1.19.0 shipped 2026-05-29, #59) — 10 phase commits. **Feature-tour overlay** (`src/tour.js`, ~200 lines, no Shepherd.js): spotlights `data-ck-tour` elements, "Press . for a quick tour" auto-prompt, `?tour=` force-start, per-user dismissal in **migration 0029** `user_tour_state`; `/onboarding` replay page. **Changelog modal** on first login after upgrade (reuses user_tour_state, tour_id `changelog-<version>`, deep links into touched UI). **In-app feedback widget** (corner FAB → bug/feature/love modal; **migration 0030** `feedback`; POST /feedback + admin queue at /admin/feedback + best-effort CK_FEEDBACK_WEBHOOK relay). **First-run product tour** (wizard completion → `/scans?tour=welcome`) + **empty-state coaching v2** (EmptyStateArgs.Steps — illustration + deep-linked 3-step CTA on the scans empty). **Screenshot-grade demo seed** (`demo_seed.go` — ~150 resources across DO/AWS/GCP/K8s × 8 weekly scans → ~500 findings on an improving 64→90 trend; fixed-RNG reproducible). **Global search**: new `internal/server/search` (in-memory index over findings/resources/scans/users/waivers/settings/docs, lithammer/fuzzysearch RankMatchFold + 90-day recency weight, 60s-tick + SSE-triggered rebuild) served at `GET /api/v1/search` (cursor-paginated); new `pkg/compliancekit` surface SearchType/SearchResult/SearchResponse (api.txt). **Global search UI**: the v1.5 Cmd-K palette upgraded to grouped results + ↑/↓+Enter nav + localStorage recent searches + a "/" global shortcut + a sticky topbar search trigger. **Table 2.0** (`src/table2.js`): drag-resize / drag-reorder / pin-left / column-visibility menu, per-(user,table) layout in **migration 0031** `user_table_state` (GET/POST /tables/{id}/layout), htmx-swap re-apply, applied to the findings table. **Inline edit + bulk-actions**: **migration 0032** adds findings.triage_status + note; bulk-select bar in the page header (Acknowledge/Resolve/Waive/Assign/Export with live counts → POST /findings/bulk, each writing a v1.12 audit_log entry) + click-to-edit triage note on the detail panel. **Detail-panel polish**: drag-resize (persisted) + detach-to-permalink (HX-Request branch serves bare partial vs full chrome page) + j/k item nav; the `?` overlay gains per-page shortcuts + kbd hint badges. 1 new dep (lithammer/fuzzysearch); schema migrations through v1.19: 32. | First 10 minutes feel guided; daily search is instant; tables feel modern |
| ~~v2.0~~ ✅ | **API-surface retro — no break needed; v2.x line opened** (closed 2026-05-29, #60; **ADR-020**). Phase-0 retro audited the full 121-symbol `pkg/compliancekit` surface across v1.0→v1.19: ADR-014's additive discipline held end-to-end, the v1.0 core is untouched, and the only warts (rbac.Resource vs compliancekit.Resource naming, etc.) are cosmetic + package-qualified. **No `/v2` module cut** — the Go library keeps its v1 import path. **`v2.0.0` is tagged** as the binary/product line-opener (decoupled from the library module major: `compliancekit version`→v2.0.0; embed→v1 import path / v1.x library versions). The next genuine ABI break triggers the `/v2` cut. Shipped alongside the **v1.19.1** audit patch (serve-bootstrap MigrateUp + deploy-manifest version-drift guard). | v2.x line opens; `/v2` library cut deferred to the next real break |
| **v2.0.x** | **App performance + UI render-correctness hardening** (near-term patch series — kicked off by the v2.0.1 fix). **v2.0.1**: fixed the findings MetricCard severity row leaking raw `&lt;section …` text — the `ck-metric-card` partial used a dynamic `<{{ $tag }}>` element name that html/template's autoescaper can't analyze, so it escaped the whole element; replaced with explicit `<a>`/`<section>` branches + a regression test that pins real (unescaped) output. **Planned**: (1) a **render-correctness sweep** — load every authed page against a real-data daemon + assert no `&lt;` HTML leak + no `ZgotmplZ`, as a visual-regression CI gate so this class of bug can't recur (the v1.18 grep checks were fooled because class-name substrings appear in escaped text too); (2) **perceived-load + SSE polish** — the live-event badge can sit on "RECONNECTING" on first paint before the EventSource `onopen` flips it to "Live"; audit the reconnect/`connected`-flag UX + first-paint cost (asset size, above-the-fold skeleton) so a cold load reads as instant; (3) findings-explorer query/index review under real-fleet row counts (6k+ findings). | Every page renders clean + feels instant on real data |
...
### v1.18 ✅ — Design system & visual polish (shipped 2026-05-28)
...
### v1.19 ✅ — Onboarding 2.0 + global search + table excellence (shipped 2026-05-29)
...
**Shipped** (v1.19.0, #59, 10 phase commits): feature-tour overlay (`src/tour.js`, migration 0029 `user_tour_state`, `/onboarding`); changelog modal on first-login-after-upgrade; in-app feedback widget (migration 0030 `feedback`, /admin/feedback, CK_FEEDBACK_WEBHOOK relay); first-run product tour (wizard → `?tour=welcome`) + empty-state coaching v2 (deep-linked 3-step CTA); screenshot-grade demo seed (`demo_seed.go` — ~150 resources × 8 weekly scans → ~500 findings, improving 64→90 trend, fixed-RNG); global search index (`internal/server/search`, fuzzy + recency, `GET /api/v1/search`, new pkg/compliancekit SearchType/SearchResult/SearchResponse); global search UI (grouped Cmd-K/"/" palette, recent searches, sticky topbar trigger); Table 2.0 (`src/table2.js` — resize/reorder/pin/visibility, migration 0031 `user_table_state`, applied to findings); inline edit + bulk-actions (migration 0032 findings.triage_status + note, page-header bulk bar → POST /findings/bulk with audit_log entries, click-to-edit triage note); detail-panel polish (resize + detach permalink + j/k nav + per-page `?` shortcuts + kbd badges). 1 new dep (lithammer/fuzzysearch); schema migrations through v1.19: 32.
...
- **`docs.compliancekit.io` static site**: Hugo (or Mkdocs + Material — decision in ADR-021 at kickoff) + a vendored theme matching the v1.18 design system; multi-version selector (v1.x current / v0.x archived); client-side search via pagefind or lunr (no SaaS dep); dark mode + mobile-first responsive.
